(function () {
  ...
  form.prototype.getRequiredInputValue = function (name, type) {
    ...
    for (k in $ff) {
      $f = $ff[k];
      if (
        ((Array.isArray(type) && type.includes($f.type)) || $f.type == type) &&
        $f.name == name &&
        $f.value !== "" &&
        $f.value !== null
      )
  ...
  form.prototype.submitHandler = function (values) {
    ...
    // Configure to formspree
    x.open("POST", "https://formspree.io/f/mvodwqgr");
    x.send(data);
  ...
  new form("form01", {
    ...
    hid: "sms",
    success: "#done",
    optional: ["_honey", "_next", "_subject", "_captcha"],
  });
})();
